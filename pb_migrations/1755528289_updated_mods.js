...
migrate((app) => {
  ...
  // update collection data
  unmarshal({
    "listRule": "@request.auth.id != \"\" && (publicApprovalStatus = 'approved' || (isPublic = false && @request.auth.id = createdBy))",
    "viewRule": "@request.auth.id != \"\" && (publicApprovalStatus = 'approved' || (isPublic = false && @request.auth.id = createdBy))"
  }, collection)
